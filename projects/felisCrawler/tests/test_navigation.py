...
class TestNavigationRules(unittest.TestCase):
    ...
    def test_allowed_links(self):
        ...
        """
        response = HtmlResponse(url='https://fr.wikipedia.org/wiki/Source', body=html.encode('utf-8'))
        links = self.link_extractor.extract_links(response)
        extracted_urls = [link.url for link in links]

        self.assertIn('https://fr.wikipedia.org/wiki/Chat', extracted_urls)
        self.assertIn('https://fr.wikipedia.org/wiki/F%C3%A9lin', extracted_urls) # Scrapy encode les URLs
        self.assertIn('https://fr.wikipedia.org/wiki/Race_de_chat', extracted_urls)
        self.assertIn('https://fr.wikipedia.org/wiki/Domestication_du_chat', extracted_urls)

    def test_denied_links(self):
        ...
        self.assertEqual(len(extracted_urls), 0, f"Should not extract any links, found: {extracted_urls}")
