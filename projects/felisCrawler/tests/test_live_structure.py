...
class TestfeliscrawlerStructure(unittest.TestCase):
    ...
    def test_live_page_structure(self) -> None:
        ...
        try:
            # 1. Récupérer la page
            headers = {
                "User-Agent": "Mozilla/5.0 (compatible; feliscrawlerBot/1.0; +http://example.com/bot)"
            }
        ...
        self.assertIsNotNone(
            item["titre"], "Title selector failed - Structure changed?"
        )
        self.assertNotEqual(
            item["titre"],
            "Je n'ai pas trouvé le titre",
            "Title fallback triggered - Structure changed?",
        )

        # Contenu
        self.assertGreater(
            item["nombre_paragraphes"],
            0,
            "No paragraphs found - Content selector failed?",
        )
        self.assertGreater(
            item["longueur_contenu"],
            0,
            "Content length is 0 - Content selector failed?",
        )

        # Images (Il devrait toujours y avoir des images sur la page Chat)
        self.assertGreater(
            item["nombre_images"], 0, "No images found - Image selector failed?"
        )

        # Liens internes
        self.assertGreater(
            len(item["liens_internes"]),
            0,
            "No internal links found - Link selector failed?",
        )
