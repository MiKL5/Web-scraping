...
class TestEndToEnd(unittest.TestCase):
    def test_run_spider_process(self) -> None:
        """
        ...
        cmd = [
            ...
            "DEPTH_LIMIT=1",
            "-s",
            "CLOSESPIDER_PAGECOUNT=1",  # Arrêter après 1 page
            "-s",
            "LOG_LEVEL=ERROR",  # Garder la sortie propre
        ]
        ...
        try:
            ...
            if result.returncode != 0:
                self.fail(
                    f"Spider failed with return code {result.returncode}.\nStderr: {result.stderr}"
                )
